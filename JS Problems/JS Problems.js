...
const arrNum = [10, 20, 4, 99, 99, 45, 67];
...
console.log(findSecondLargest(arrNum));
...
console.log(findSecondSmallestNumber(arrNum));
...
const uniArr = [7, 8, 8, 9, 10, 7, 10];
const uniqArr = (arr) => {
  const uniqList = [];
  for (let i = 0; i < arr.length; i++) {
    if (!uniqList.includes(arr[i])) uniqList.push(arr[i]);
  }
  return uniqList;
};
console.log(uniqArr(uniArr));

const inputArr = [13, 14, 15, 16, 13, 14];
...
console.log(removeDuplicateFilter(inputArr));
...
console.log(removeDuplicateReduce(inputArr));
